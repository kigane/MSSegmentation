...
all_slices = sorted(glob.glob(BASE_DIR+'train/*.h5'))
print(len(all_slices))
# pprint(all_slices)

test_slices = [extract_casename(x) for x in all_slices if 'patient05' in x]
train_slices = [extract_casename(x) for x in all_slices if 'patient05' not in x]
...
with open('./data/isbi2015raw/train_slices.list', 'w') as f:
    for l in train_slices:
        f.write(l)
        f.write('\n')

with open('./data/isbi2015raw/test.list', 'w') as f:
