...
#         return x[:, :, pad:-pad, pad:-pad]
    ...
    def forward(self, x, H, W):
        ...
        # [BCHW] => [B C/self.shift_size HW] * self.shift_size
        xs = torch.chunk(xn, self.shift_size, 1)  # embed=160 => 5*32
        x_shift = [torch.roll(x_c, shift, 2)  # 每个chunk中特征图移位
                   for x_c, shift in zip(xs, range(-self.pad, self.pad+1))]
        x_cat = torch.cat(x_shift, 1)
        x_cat = torch.narrow(x_cat, 2, self.pad, H)  # 去pad
